...
// ✅ força tipagem “solta” só aqui (resolve o vermelho do TS)
...
export async function listTransactionsByMonth(householdId: string, monthKey?: string | Date) {
 ...
 */
export const listTransactions = (householdId: string, arg?: string | Date | number) => {
  if (typeof arg === "number") return listTransactionsRecent(householdId, arg);
  return listTransactionsByMonth(householdId, arg);
};

export async function getMonthlyNet(householdId: string, monthKey?: string | Date) {
  const { start, end } = monthRange(monthKey);
...
export async function getNetBetween(householdId: string, startYMD: string, endYMD: string) {
  const { data, error } = await sb
    .from("transactions")
    .select("type,amount_cents,occurred_on")
    .eq("household_id", householdId)
...
// ✅ NOVO: saldo “total/atual” (acumulado)
export async function getAllTimeNet(householdId: string) {
  const start = "1970-01-01";
  const end = ymd(addDays(new Date(), 1)); // amanhã (exclusivo)
  return getNetBetween(householdId, start, end);
}